...
const { CinematicVideoService } = require('./cinematic-video-service');
...
class AIVideoGenerator {
  constructor(credentials, options = {}) {
    ...
    this.mediaGeneration = options.mediaGeneration || (this.db
      ...
      : null);
    this.cinematicVideo = options.cinematicVideo || new CinematicVideoService({ logger: this.logger });
  }
    ...
    try {
      ...
      if (this.elevenLabsApiKey && this.elevenLabsVoiceId) {
        ...
        try {
          ...
          if (this.openai && !process.env.OPENAI_BASE_URL) {
            provider = 'openai';
  ...
  async generateVideo(script, visualAssets, audioPath, outputPath, options = {}) {
    ...
    try {
      ...
      const providerSetting = String(process.env.VIDEO_PROVIDER || 'realistic_cinematic').toLowerCase();
      if (providerSetting === 'realistic_cinematic' || providerSetting === 'cinematic_broll') {
        const produced = await this.cinematicVideo.render(script, audioPath, outputPath, options);
        this.lastVideoResult = {
          requestedProvider: 'realistic_cinematic',
          actualProvider: 'realistic_cinematic',
          model: 'cinematic-broll-ffmpeg',
          mode: 'cinematic_broll',
          generatedSeconds: options.estimatedDuration || this.calculateScriptDuration(script),
          tasks: [],
          scenes: []
        };
        return produced;
