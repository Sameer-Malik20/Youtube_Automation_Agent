...
class PublishingSchedulingAgent {
  ...
  async uploadToYouTube(scheduleEntry) {
    ...
    const isScheduled = Boolean(scheduleEntry.publishTime && new Date(scheduleEntry.publishTime) > new Date());
    const privacyStatus = isScheduled
      ? 'private'
      : (metadata.privacyStatus || process.env.DEFAULT_PRIVACY_STATUS || 'public');
    ...
    const videoMetadata = {
      ...
      status: {
        ...
        ...(isScheduled ? { publishAt: new Date(scheduleEntry.publishTime).toISOString() } : {}),
